<!DOCTYPE html>
<html lang="en">
    <?php
        require_once("../../../../session.php");
        require_once("../../../../js_css_header.php");
    ?>
    <body>
        <div id="app" class="app app-header-fixed app-sidebar-fixed">
            <?php
                require_once('../../../../navbar.php');
                require_once("../../../../menu.php");
            ?>
            <div class="app-sidebar-bg" data-bs-theme="dark"></div>
            <div class="app-sidebar-mobile-backdrop"><a href="#" data-dismiss="app-sidebar-mobile" class="stretched-link"></a></div>
            <div id="content" class="app-content">
                <h1 class="page-header mb-3">Revise Selling List</h1>
                <div class="panel panel-inverse">
					<div class="panel-heading">
						<h4 class="panel-title">DataTable - Revise Selling</h4>
						<?php require_once("../../../../comp_ui/panel-header.php"); ?>
					</div>
                    <div class="panel-body">
                        <table id="table_revise_selling" class="table table-bordered" style="width: 100%;">
                            <thead>
                                <tr>
                                    <th class="text-nowrap">#</th>
                                    <th class="text-nowrap">Details / Actions</th>
                                    <th class="text-nowrap">Status</th>
                                    <th class="text-nowrap">Content</th>
                                    <th class="text-nowrap">Request Datetime</th>
                                    <th class="text-nowrap">Request By</th>
                                    <th class="text-nowrap">Remarks</th>
                                    <th class="text-nowrap">Approve Datetime</th>
                                    <th class="text-nowrap">Approve By</th>
                                </tr>
                            </thead>
                            <tbody class="fw-600"></tbody>
                        </table>
                    </div>
                </div>
            </div>
            <a href="javascript:;" class="btn btn-icon btn-circle btn-theme btn-scroll-to-top" data-toggle="scroll-to-top"><i class="fa fa-angle-up"></i></a>
        </div>
    </body>
    <?php require_once("../../../../js_css_footer.php"); ?>
</html>
<div id="load_view_detail" class="modal fade"></div>


<script type="text/javascript">
    $(document).ready(function(){
        FilterData('#table_revise_selling')
    })

    function FilterData(tbl){
        Swal.fire({
            title: 'กรุณารอสักครู่...',
            text: 'กำลังดำเนินการแสดงผลข้อมูล',
            imageUrl: 'lib.albatrosslogistic.com/assets/gif/ajax-loader.gif',
            showConfirmButton: false,
            showCancelButton: false,
            didOpen: () => {
                $.post('<?=$CFG->func_bom_issue?>/management', { protocol: 'ListReviseSelling' }, function(data){
                    console.log(data)
                    try {
                        var result = JSON.parse(data)

                        $(tbl+' thead tr').clone(true).appendTo(tbl+' thead')
                        $(tbl+' thead tr:eq(0) th').each(function(i){
                            var title = $(this).text()
                            if(i > 2){
                                $(this).html('<input type="text" placeholder="Search by '+ title +'" />')
                                $('input', this).on('keyup change', function(){
                                    if(table.column(i).search() !== this.value){
                                        table.column(i).search(this.value).draw()
                                    }
                                })
                            }else{
                                $(this).html('')
                            }
                        })

                        var table = $(tbl, i = 0).DataTable({
                            dom: '<"dataTables_wrapper dt-bootstrap"<"row"<"col-lg-8 d-block d-sm-flex d-lg-block justify-content-center"<"d-block d-lg-inline-flex me-0 me-md-3"l><"d-block d-lg-inline-flex"B>><"col-lg-4 d-flex d-lg-block justify-content-center"fr>>t<"row"<"col-md-5"i><"col-md-7"p>>>',
                            buttons: [
                                { extend: 'copy', className: 'btn-sm' },
                                { extend: 'csv', className: 'btn-sm' },
                                { extend: 'excel', className: 'btn-sm' },
                                { extend: 'pdf', className: 'btn-sm' },
                            ],
                            colReorder: true,
                            keys      : true,
                            rowReorder: true,
                            select    : true,
                            scrollX   : true,
                            bDestroy  : true,
                            lengthMenu  : [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] ],
                            pageLength  : 100,
                            scrollY   : '480px',
                            data      : result.datas,
                            columns : [
                                { data: function(data){ i++; return i }, className: "text-nowrap text-center" },
                                { data: function(data){
                                    var disab = '<?=$mrp_user_code_mst?>' == 'GDJ00258' && data.rev_status == 'Pending' ? '' : 'disabled'
                                    var olwne = '<?=$mrp_user_name_mst?>' == data.rev_by && data.rev_status == 'Pending' ? '' : 'disabled'
                                    return '<button '+disab+' onclick="OpenViewDetail(\'#load_view_detail\', \'load_approve_selling\', \''+data.rev_uniq+'\')" class="btn btn-xs bg-gradient-blue fw-600 text-white"><i class="fa-solid fa-pencil"></i> Approve</button>' +
                                           '<button onclick="OpenViewDetail(\'#load_view_detail\', \'load_view_selling\', \''+data.rev_uniq+'\')" class="btn btn-xs bg-gradient-black fw-600 ms-1 text-white"><i class="fa-solid fa-pencil"></i> View Detail</button>' +
                                           '<button '+olwne+' onclick="CancelRequest(\''+data.rev_uniq+'\')" class="btn btn-xs bg-gradient-red fw-600 ms-1 text-dark"><i class="fa-solid fa-ban"></i> Cancel</button>' 
                                }, className: "text-nowrap text-center" },
                                { data: function(data){ return '<span class="badge rounded fw-600 text-white '+data.class_color+'">'+data.rev_status+'</span>' }, className: "text-nowrap text-center" },
                                { data: function(data){ return data.rev_content }, className: "text-nowrap" },
                                { data: function(data){ return moment(data.rev_datetime).format('DD/MM/YYYY') }, className: "text-nowrap" },
                                { data: function(data){ return data.rev_by }, className: "text-nowrap" },
                                { data: function(data){ return data.rev_remarks }, className: "text-nowrap" },
                                { data: function(data){ return data.rev_aprv_datetime ? moment(data.rev_aprv_datetime).format('DD/MM/YYYY') : '' }, className: "text-nowrap" },
                                { data: function(data){ return data.rev_aprv_by }, className: "text-nowrap" },
                            ]
                        }).draw(false)
                    } catch(err) {
                        SwalOnlyText('warning', 'ไม่สามารถดำเนินการได้ ' + err.message)
                    }

                    Swal.fire({
                        icon: 'success',
                        text: 'แสดงผลข้อมูลสำเร็จ',
                        showConfirmButton: false,
                        showCancelButton: false,
                        timer: 1000,
                    })
                })
            }
        })
    }

    function CancelRequest(rev_uniq){
        Swal.fire({
            icon: 'info',
            text: 'ยืนยันการยกเลิก Request update selling price หรือไม่?',
            input: 'text',
            inputAttributes: { autocapitalize: 'off' },
            showCancelButton: true,
            confirmButtonText: '<i class="fas fa-thumbs-up"></i> ตกลง!',
            showLoaderOnConfirm: true,
            allowOutsideClick: () => !Swal.isLoading(),
            preConfirm: (rem) => {
                if(rem == ''){
                    Swal.showValidationMessage('กรุณากรอกเหตุผลในการยกเลิกรายการ')
                }
            } 
        }).then((thens) => {
            if(thens.isConfirmed){
                Swal.fire({
                    title: 'กำลังดำเนินการบันทึกข้อมูล...',
                    text: 'กรุณารอสักครู่',
                    imageUrl: '<?=$CFG->sub_gif?>/ajax-loader.gif',
                    showConfirmButton: false,
                    showCancelButton: false,
                    didOpen: () => {
                        $.post("<?=$CFG->func_bom_issue?>/management", { protocol: 'CancelRequest', rev_uniq: rev_uniq, remarks: thens.remarks }, function(data){
                            try {
                                const result = JSON.parse(data)

                                if(result.code == 200){
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'ดำเนินการสำเร็จ',
                                        text: result.message,
                                    }).then(() => {
                                        location.reload()
                                    })
                                }else{
                                    SwalOnlyText('warning', result.message)
                                }
                            } catch(e) {
                                SwalOnlyText('error', 'ไม่สามารถดำเนินการได้ ' + e.message)
                            }
                        })
                    }
                })
            }
        })
    }
</script>