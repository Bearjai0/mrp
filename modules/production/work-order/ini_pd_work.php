    ...
    <body>
        <div id="app" class="app app-header-fixed app-sidebar-fixed">
            ...
            <div id="content" class="app-content">
                <h1 class="page-header mb-3">Production work order</h1>
                <div class="panel panel-inverse">
					<div class="panel-heading">
						<h4 class="panel-title">DataTable - Work order</h4>
					...
					<div class="panel-body">
                        <table id="table_pd_work" class="table table-bordered" style="width: 100%;">
                            <thead>
                                <tr>
...
<script type="text/javascript">
    $(document).ready(function(){
        calldata('#table_pd_work')
    })

    function calldata(tbl){
        $.post('<?=$CFG->fol_pd_work?>/management', { protocol: 'SyncPendingPlan', }, function(data){
            try {
                ...
                var table = $(tbl).DataTable({
                    ...
                    columns : [
                        ...
                        { data: function(data){
                            return '<button onclick="OpenViewDetail(\'#load_view_detail\', \'<?=$CFG->fol_pd_work?>/load_pd_confirm_work\', \''+data.job_no+'\')" class="btn btn-icon rounded bg-gradient-blue-indigo text-white" data-toggle="tooltip" data-placement="top" title="Confirm Job"><i class="fa-solid fa-thumbs-up"></i></button>' +
                                   '<button onclick="OpenViewDetail(\'#load_view_detail\', \'<?=$CFG->fol_pd_work?>/load_pd_confirm_machine\', \''+data.job_no+'\')" class="btn btn-icon rounded bg-gradient-blue-purple text-white ms-2" data-toggle="tooltip" data-placement="top" title="Confirm Machine"><i class="fa-solid fa-screwdriver-wrench"></i></button>' +
                                   ...
                                   '<button onclick="RejectJob(\''+data.job_no+'\')" class="btn btn-icon rounded bg-gradient-yellow-red text-white ms-2" data-toggle="tooltip" data-placement="top" title="Reject job"><i class="fa-solid fa-thumbs-down"></i></button>'
    ...
    function RejectJob(job_no){
        Swal.fire({
            icon: 'info',
            title: 'Reject แผนการผลิต',
            text: 'ต้องการ Reject JOB NO '+ job_no +' หรือไม่? รายการ Raw-Material ที่ใช้ในการผลิตจำกลับไปรอรับเข้า Inventory อีกครั้ง',
            ...
            preConfirm: (checkVal) => {
                if(checkVal == ''){
                    Swal.showValidationMessage('กรุณากรอกเหตุผลในการ​ Reject JOB No# นี้')
                }
            } 
        }).then((thens) => {
            if(thens.isConfirmed){
                $.post('<?=$CFG->fol_pd_work?>/management', { protocol: 'RejectPlan', job_no: job_no, remarks: thens.value }, function(data){
                    console.log(data)
                    try {
                        const result = JSON.parse(data)
                        if(result['code'] == '200'){
