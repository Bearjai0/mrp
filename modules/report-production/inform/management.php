    ...
    if($protocol == "SummaList"){
        try {
            ...
            $list = $db_con->prepare(
                ...
                LEFT JOIN tbl_bom_mst AS C ON A.job_bom_id = C.bom_id OR A.job_bom_id = C.bom_uniq
            ...
            $list->bindParam(':start_date', $start_date);
            ...
            while($listResult = $list->fetch(PDO::FETCH_ASSOC)){
                if($listResult['job_bom_id'] == 'FG0001-03090'){
                    ...
                    $result['cost_total_oh'] = $cqlResult['unit_price'];
        ...
        try {
            ...
            if($start_date == $end_date && $job_status == ""){
                $sql = "SELECT TOP(1000) ROW_NUMBER() OVER(ORDER BY A.job_no DESC) AS list,
                                        ...
                                        A.job_ft2_perpage, A.job_ft2_usage, 
                                        B.job_ref,
                                        C.class_color
                        FROM tbl_job_mst AS A
